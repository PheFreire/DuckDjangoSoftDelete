...
test-pg: ## run tests on ephemeral PostgreSQL via Testcontainers (Docker required)
	USE_TESTCONTAINERS=1 poetry run python -m pytest -q

test-cov: ## run tests with coverage
	poetry run python -m pytest --cov=src/duck_django_soft_delete --cov-report=term-missing

test-parallel: ## run tests in parallel (needs pytest-xdist)
	poetry run python -m pytest -n auto -q
